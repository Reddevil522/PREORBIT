...
import { osChapters, osTests } from '../../../config/core-cs.config';
...
@Component({
  ...
  styleUrl: './operating-system.component.css'
})
export class OperatingSystemComponent implements OnInit {
  totalChapters = osChapters.length;
  ...
  ngOnInit() {
    ...
    this.testEngineService.getTestSummary('core-cs', 'operating-system').subscribe({
      next: (summary) => {
