...
const OOPQuestionSchema = new mongoose.Schema({
  question: {
    type: String,
  ...
  chapterSlug: {
    ...
    enum: {
      values: oopChapters,
      message: 'Chapter slug is not valid for OOP'
    }
  },
  testId: {
    type: String,
...
OOPQuestionSchema.index({ chapterSlug: 1 });
OOPQuestionSchema.index({ testId: 1 });
OOPQuestionSchema.index({ chapterSlug: 1, testId: 1 });

module.exports = mongoose.models.OOPQuestion || mongoose.model('OOPQuestion', OOPQuestionSchema);
