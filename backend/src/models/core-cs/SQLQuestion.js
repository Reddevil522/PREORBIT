...
const SQLQuestionSchema = new mongoose.Schema({
  ...
  chapterSlug: {
    ...
    enum: {
      values: sqlChapters,
      message: 'Chapter slug is not valid for SQL'
    }
  },
  testId: {
    type: String,
...
SQLQuestionSchema.index({ chapterSlug: 1 });
SQLQuestionSchema.index({ testId: 1 });
SQLQuestionSchema.index({ chapterSlug: 1, testId: 1 });

module.exports = mongoose.models.SQLQuestion || mongoose.model('SQLQuestion', SQLQuestionSchema);
