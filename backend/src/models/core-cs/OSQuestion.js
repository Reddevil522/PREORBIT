...
const OSQuestionSchema = new mongoose.Schema({
  ...
  chapterSlug: {
    ...
    enum: {
      values: osChapters,
      message: 'Chapter slug is not valid for OS'
    }
  },
  testId: {
    type: String,
...
OSQuestionSchema.index({ chapterSlug: 1 });
OSQuestionSchema.index({ testId: 1 });
OSQuestionSchema.index({ chapterSlug: 1, testId: 1 });

module.exports = mongoose.models.OSQuestion || mongoose.model('OSQuestion', OSQuestionSchema);
