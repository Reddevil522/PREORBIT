...
const DBMSQuestionSchema = new mongoose.Schema({
  ...
  chapterSlug: {
    ...
    enum: {
      values: dbmsChapters,
      message: 'Chapter slug is not valid for DBMS'
    }
  },
  testId: {
    type: String,
...
DBMSQuestionSchema.index({ chapterSlug: 1 });
DBMSQuestionSchema.index({ testId: 1 });
DBMSQuestionSchema.index({ chapterSlug: 1, testId: 1 });

module.exports = mongoose.models.DBMSQuestion || mongoose.model('DBMSQuestion', DBMSQuestionSchema);
