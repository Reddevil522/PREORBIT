const SQLQuestionService = require('../../services/core-cs/SQLQuestionService');
const { sendSuccess, sendError } = require('../../utils/response');

exports.getQuestionsForTest = async (req, res, next) => {
  try {
    const { chapterSlug, testId } = req.params;
    const questions = await SQLQuestionService.getQuestionsForTest(chapterSlug, testId);
...
exports.getQuestionsByTestAdmin = async (req, res, next) => {
  try {
    const { chapterSlug, testId } = req.params;
    const questions = await SQLQuestionService.getQuestionsByTest(chapterSlug, testId);
...
exports.getQuestionById = async (req, res, next) => {
  try {
    const question = await SQLQuestionService.getQuestionById(req.params.id);
...
exports.createQuestion = async (req, res, next) => {
  try {
    const question = await SQLQuestionService.createQuestion(req.body);
...
exports.updateQuestion = async (req, res, next) => {
  try {
    const question = await SQLQuestionService.updateQuestion(req.params.id, req.body);
    if (!question) return sendError(res, 404, 'Question not found');
...
exports.deleteQuestion = async (req, res, next) => {
  try {
    const question = await SQLQuestionService.deleteQuestion(req.params.id);
    if (!question) return sendError(res, 404, 'Question not found');
