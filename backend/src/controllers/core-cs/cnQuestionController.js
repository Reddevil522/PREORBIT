const CNQuestionService = require('../../services/core-cs/CNQuestionService');
const { sendSuccess, sendError } = require('../../utils/response');

exports.getQuestionsForTest = async (req, res, next) => {
  try {
    const { chapterSlug, testId } = req.params;
    const questions = await CNQuestionService.getQuestionsForTest(chapterSlug, testId);
...
exports.getQuestionsByTestAdmin = async (req, res, next) => {
  try {
    const { chapterSlug, testId } = req.params;
    const questions = await CNQuestionService.getQuestionsByTest(chapterSlug, testId);
...
exports.getQuestionById = async (req, res, next) => {
  try {
    const question = await CNQuestionService.getQuestionById(req.params.id);
...
exports.createQuestion = async (req, res, next) => {
  try {
    const question = await CNQuestionService.createQuestion(req.body);
...
exports.updateQuestion = async (req, res, next) => {
  try {
    const question = await CNQuestionService.updateQuestion(req.params.id, req.body);
    if (!question) return sendError(res, 404, 'Question not found');
...
exports.deleteQuestion = async (req, res, next) => {
  try {
    const question = await CNQuestionService.deleteQuestion(req.params.id);
    if (!question) return sendError(res, 404, 'Question not found');
